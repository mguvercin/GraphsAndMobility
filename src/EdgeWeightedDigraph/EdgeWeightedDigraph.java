...
public class EdgeWeightedDigraph implements Serializable {
	...
	public EdgeWeightedDigraph(int V, int E) {
		...
		for (int i = 0; i < E; i++) {
			...
			int weight = (int)Math.round(100 * Math.random()) ;// dont do
																	// this
			DirectedEdge e = new DirectedEdge(v, w, weight, 1,1,1);
			addEdge(e);
		}
	}
	...
	public EdgeWeightedDigraph(In in) {
		...
		for (int i = 0; i < E; i++) {
			...
			int weight = 1;
			addEdge(new DirectedEdge(v, w, weight, 1,1,1));
			addEdge(new DirectedEdge(w, v, weight, 1,1,1));
		}
	}

	public EdgeWeightedDigraph(In in, int weight, boolean i0) {
		this(in.readInt());
		int E = in.readInt();
		if (E < 0)
			throw new IllegalArgumentException(
	...
	public EdgeWeightedDigraph(In in, boolean doesStartFromZero) {
		this(in.readInt());
		int E = in.readInt();
		if (E < 0)
			throw new IllegalArgumentException(
		...
		//	double weight = in.readDouble();
			// weight = 0.5;
			addEdge(new DirectedEdge(v, w, 1,1,1,1));
		}
	}
	...
	public DirectedEdge getEdgeOf(int v, int w) {// w is to v is from
		Bag<DirectedEdge> list = adj[v];
