...
public class DateTimeModified implements Serializable {
	private int hour;
	private int minute;
	private int second;
	...
	public static DateTimeModified parseTime(String s) {
		// TODO regex control and exception
		String[] time = s.split(":");
		return new DateTimeModified(Integer.parseInt(time[0]), Integer.parseInt(time[1]), Integer.parseInt(time[2]));
	}

	// at minute
	public double difference(DateTimeModified other) {
		...
		return h * 60 + min + sec / 60.0;
	}

	public double absDifference(DateTimeModified other) {
		int sec = this.second - other.second;
		int min = this.minute - other.minute;
		int h = this.hour - other.hour;
		if (sec < 0) {
	...
	public DateTimeModified(int hour, int minute, int second) {
		super();
		this.hour = hour;
		this.minute = minute;
		this.second = second;
